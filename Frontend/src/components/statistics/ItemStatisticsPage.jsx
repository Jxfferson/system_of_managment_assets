...
import { Calendar, Box, CheckCircle2, ArrowUpRight, BarChart3, Table2, Clock, X, AlertCircle, AlertTriangle, Download } from 'lucide-react';
import { 
...
const OrderModal = ({ isOpen, onClose, item, available, total }) => {
  ...
  useEffect(() => {
    if (isOpen && item && orderItems.length === 0) {
      setOrderItems([{
        ...
        total: total,
        available: available,
        unitPrice: getSuggestedPrice(item),
        quantity: 0,
      }]);
    }
  }, [isOpen, item, available, total]);

  const updateQuantity = (quantity) => {
    setOrderItems([{ ...orderItems[0], quantity: Math.max(1, quantity) }]);
  };

  const updateUnitPrice = (unitPrice) => {
    setOrderItems([{ ...orderItems[0], unitPrice: Math.max(0, unitPrice) }]);
  };

  const currentItem = orderItems[0];
  const calculateItemTotal = () => currentItem?.quantity * currentItem?.unitPrice || 0;
  ...
  const exportToExcel = () => {
    ...
    const exportData = [{
      ...
      'Precio Unitario (COP)': `$ ${currentItem.unitPrice.toLocaleString('es-CO')}`,
      'Total (COP)': `$ ${calculateItemTotal().toLocaleString('es-CO')}`
    }];

    exportData.push({
      '#': '', 'Item': 'TOTAL GENERAL', 'Total en Inventario': '', 'Disponibles': '',
      'Cantidad a Pedir': currentItem.quantity, 'Precio Unitario (COP)': '', 'Total (COP)': `$ ${calculateItemTotal().toLocaleString('es-CO')}`
    });

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Orden de Compra');
    ws['!cols'] = [{ wch: 5 }, { wch: 40 }, { wch: 20 }, { wch: 15 }, { wch: 18 }, { wch: 25 }, { wch: 25 }];
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-auto rounded-2xl bg-slate-900 border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-white/10 bg-slate-900/95 backdrop-blur">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Single item order</p>
          </div>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) => updateUnitPrice(parseInt(e.target.value) || 0)}
                ...
                <tr>
                  ...
                  <td colSpan="2" className="px-4 py-4 text-right">
                    <span className="text-2xl font-bold text-cyan-400">
                      ${calculateItemTotal().toLocaleString('es-CO')} COP
                    </span>
                  </td>
                </tr>
              </tfoot>
